...
ADD_ARCHIVE_FILE_SCRIPT="${GITHUB_DIR}/add_archive_file/add_archive_file.sh"
GET_UBUNTU_IMAGES_SCRIPT="${GITHUB_DIR}/get_ubuntu_images/get_ubuntu_images.sh"
...
main() {
    ...
    # Get the base images
    declare -A base_images="( $($GET_UBUNTU_IMAGES_SCRIPT main) )"
    ...
    for key in "${!base_images[@]}"; do
        ...
        _suffix="${_name#*.}" # remove up to, and including, the first '.'
